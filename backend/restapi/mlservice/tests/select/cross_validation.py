...
# アヤメデータの読み込み
iris_data = pd.read_csv("/backend/restapi/mlservice/data/iris.csv", encoding="utf-8")
...
for(name, algorithm) in allAlgorithms:
    if name == 'CheckingClassifier':
        continue
    if name == 'ClassifierChain':
        continue
    if name == 'MultiOutputClassifier':
        continue
    if name == 'OneVsOneClassifier':
        continue
    if name == 'OneVsRestClassifier':
        continue
    if name == 'OutputCodeClassifier':
        continue
